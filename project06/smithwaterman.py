...
def smith_waterman(seq1, seq2, match=2, mismatch=-1, gap=-1):
    ...
    # Get the highest alignment score from the scoring matrix
    max_score = np.max(score_matrix)

    # Get the highest alignment score if both sequences were identical and use
    # shorter sequence
    self_score = min(len(seq1), len(seq2)) * match
